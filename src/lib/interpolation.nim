...
proc calculateLabDistance(color1, color2: LabColor): float =
  sqrt(pow(color1[0] - color2[0], 2) + 
       pow(color1[1] - color2[1], 2) + 
       pow(color1[2] - color2[2], 2))

proc isColorSufficientlyDifferent(newColor: LabColor, existingColors: seq[LabColor], 
                                 minDistance: float = 0.05): bool =
...
proc generateLabVariation(baseColor: LabColor, variationType: int, 
                         intensity: float = 0.1): LabColor =
  result = baseColor
  
  case variationType mod 8:
  of 0: result[0] = clamp(baseColor[0] + intensity, 0.0, 1.0)
  of 1: result[0] = clamp(baseColor[0] - intensity, 0.0, 1.0)
  of 2: result[1] = clamp(baseColor[1] + intensity, 0.0, 1.0)
  of 3: result[1] = clamp(baseColor[1] - intensity, 0.0, 1.0)
  of 4: result[2] = clamp(baseColor[2] + intensity, 0.0, 1.0)
  of 5: result[2] = clamp(baseColor[2] - intensity, 0.0, 1.0)
  of 6:
...
proc addInterpolatedColors(labPalette: seq[LabColor], expandedLab: var seq[LabColor], 
                          targetCount: int) =
  if labPalette.len < 2: return
  
  var sortedIndices = toSeq(0..<labPalette.len)
  sortedIndices.sort(proc(a, b: int): int =
    if labPalette[a][0] < labPalette[b][0]: -1
    elif labPalette[a][0] > labPalette[b][0]: 1
    else: 0
  )
  
  let interpolationsNeeded = min(targetCount - labPalette.len, 
                                (labPalette.len - 1) * 3)
...
proc expandPalette(originalColors: seq[string], targetCount: int = 256): seq[string] =
  ...
  echo "Expanded palette from ", originalColors.len, " to ", result.len, " colors using LAB color space"
