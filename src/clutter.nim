import std/[sequtils, math, algorithm, os, tables, hashes]
import lib/palettes
include lib/vips
...
proc findBestPaletteMatch(
    inputRgb: array[3, float], palette: seq[Color]
): array[3, float] =
  ...
  for paletteColor in palette:
    ...
    let hueDiff =
      min(abs(inputHsv[0] - paletteHsv[0]), 1.0 - abs(inputHsv[0] - paletteHsv[0]))
...
proc generatePaletteLUT(
    targetColors: seq[string], strength: float = 0.8
): ptr VipsImage =
  let lutSize = 256
  ...
  for i in 0 ..< lutSize:
    ...
    for channel in 0 .. 2:
      ...
      finalRgb[channel] =
        originalColor[channel] * (1.0 - blendFactor) +
...
proc applyPaletteLUT(
    image: ptr VipsImage,
    lut: ptr VipsImage,
    preserveDetails: bool = true,
    detailStrength: float = 0.3,
): ptr VipsImage =
  ...
  let blurStatus = vips_resize(image, detailMask.addr, 0.5, nil)
    # Simple detail detection
  ...
  let linearStatus = vips_linear(
    lutResult, preservedResult.addr, a[0].unsafeAddr, b[0].unsafeAddr, 3, nil
  )
...
proc processImageWithPaletteGrading(
    inputPath: string,
    outputPath: string,
  ...
  try:
    ...
    checkVipsResult(
      vips_image_write_to_file(outputImage, outputPath.cstring, nil), "write_to_file"
    )
...
proc clutter(
  ...
  try:
    let pal = parsePalettes()
    var colors = palette
    if palette.len == 1 and palette[0] in pal:
      colors = pal[palette[0]]
    echo "Processing with palette strength: ", strength
    processImageWithPaletteGrading(
      input,
      output,
      colors.expandPalette(interpolation),
      strength = strength,
...
when isMainModule:
  ...
  installPalettes()
  if palettescmd.add.seen:
    echo "Creating palette '" & add.name.value & "'"
    addPalette(add.name.value, add.colors.value.split(" "))
  if args.input.value != "":
    var outPath = args.output.value
    if outPath == "":
      outPath =
        $hash(args.input.value & args.palette.value & splitFile(args.input.value).ext)
    clutter(
      args.input.value,
      outPath,
      args.strength.value,
      args.interp.value,
      args.palette.value.split(" "),
    )
